...
module.exports = {
  ...
  listItems: function(type, items, listsDir) {
    var listDbFile = _path.resolve(listsDir + './list-' + type + '.db');
  ...
  getListItemsID: function getListNumber(type, number, listsDir) {
    var listDbFile = _path.resolve(listsDir + './list-' + type + '.db');
    var listDb = new Datastore({ filename: listDbFile});
    return listDb.loadDatabaseAsync().then(function(){
