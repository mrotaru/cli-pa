  ...
  var commands = {
    ...
    'list': function() {
      ...
      return core.db.read({type: 'duckrow'}).then(function(rows){
        ...
        var dbOps = [];
        var ducks = [];
        for (var i=0; i < rows[0].ducks.length; ++i) {
          var listIndex = rows[0].ducks[i].listIndex;
          dbOps.push(core.db.read({_id: rows[0].ducks[i].id}).then(function(res){
            if(res.length === 1) {
              debug('in prom', duck, ducks, i);
              res[0].listIndex = listIndex;
              ducks.push(res[0]);
            }
          }));
        }

        Promise.all(dbOps).then(function(){
          debug('after all: ', ducks);
        });

        for (var i=0; i < rows[0].ducks.length; ++i) {
          var duck = rows[0].ducks[i];
          var c = 0;
          var out = '\u001b[3' + c + ';1m' + duck.listIndex + '. ' + duck.title
          + '\u001b[0m';
          console.log(out);
        }
      });
